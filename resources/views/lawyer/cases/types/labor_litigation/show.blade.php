            ...
            @if($caseTypeData)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                    <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-sm">
                            <div>
                                <dt class="font-medium text-gray-500">Claim Type</dt>
                                <dd class="text-gray-900">{{ $caseTypeData->claim_type ?? '—' }}</dd>
                            </div>
                            <div>
                                <dt class="font-medium text-gray-500">Claim Amount</dt>
                                <dd class="text-gray-900">ETB {{ number_format($caseTypeData->claim_amount ?? 0, 2) }}</dd>
                            </div>
                            <div>
                            ...
                            @if($caseTypeData->early_settled)
                                <div>
                                    <dt class="font-medium text-gray-500">Early Settlement</dt>
                            ...
                            @endif
                            @if($caseTypeData->execution_opened_at)
                                <div>
                                    <dt class="font-medium text-gray-500">Execution Opened At</dt>
                                    <dd class="text-gray-900">{{ \Carbon\Carbon::parse($caseTypeData->execution_opened_at)->format('M d, Y') }}</dd>
                                </div>
                            @endif
                        </div>

                        @if($caseTypeData->claim_material_desc)
                            <div class="mt-6">
                                <h5 class="text-sm font-medium text-gray-500 mb-2">Material Description</h5>
                                <p class="text-base text-gray-900 whitespace-pre-line">{{ $caseTypeData->claim_material_desc }}</p>
                            </div>
                        @endif
                    </div>
                </div>
