...
<head>
    <meta charset="utf-8">
    <title>Secured Loan Recovery Cases Report - Legal Organization</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="document-container">
                ...
                <tr>
                    ...
                    <td class="header-right-cell">
                        <div class="header-right">
                            <div class="report-title">Secured Loan Recovery Report</div>
                            <div class="report-subtitle">Comprehensive Secured Loan Recovery Case Analysis</div>
                        </div>
                    </td>
                </tr>
            </table>
        ...
        <div class="content-section">
            <div class="description">
                <p class="description-text">
                    This comprehensive report lists all secured loan recovery cases, including parties involved, outstanding and recovered amounts, current status, and related branch/court information. The report focuses on secured asset recovery performance analysis to support strategic financial decision-making.
                </p>
            </div>
            ...
            <div class="table-container">
                <div class="table-header">
                    <h3 class="table-title">Secured Loan Recovery Cases Data</h3>
                </div>
        ...
        <thead>
            <tr>
                ...
                <th>Base Branch</th>
                <th>Customer Name</th>
                <th>Outstanding Claimed (ETB)</th>
                <th>Latest Status</th>
                <th>Amount Recovered (ETB)</th>
                <th>Performance %</th>
                <th>Warning Issued</th>
                <th>Collateral Estimated</th>
                <th>Auction Published</th>
                <th>Sold or Acquired</th>
            </tr>
        </thead>
        <tbody>
        @foreach($cases as $c)
            ...
            <tr>
                ...
                <td class="text-center">
                    @if(isset($c->warning_issued) && $c->warning_issued)
                        <span class="status-badge status-completed">Y</span>
                    @else
                        <span class="status-badge status-pending">N</span>
                    @endif
                </td>
                <td class="text-center">
                    @if(isset($c->collateral_estimated) && $c->collateral_estimated)
                        <span class="status-badge status-completed">Y</span>
                    @else
                        <span class="status-badge status-pending">N</span>
                    @endif
                ...
                <td class="text-center">
                    @if(isset($c->auction_published) && $c->auction_published)
                        <span class="status-badge status-completed">Y</span>
                    @else
                        <span class="status-badge status-pending">N</span>
                    @endif
                ...
                <td>
                    @if(isset($c->sold_or_acquired_status))
                        @if(strtolower($c->sold_or_acquired_status) === 'sold')
                            <span class="status-badge status-completed">Sold</span>
                        @elseif(strtolower($c->sold_or_acquired_status) === 'acquired')
                            <span class="status-badge status-in-progress">Acquired</span>
                        @else
                            <span class="status-badge status-pending">{{ ucfirst($c->sold_or_acquired_status) }}</span>
                        @endif
                    @else
